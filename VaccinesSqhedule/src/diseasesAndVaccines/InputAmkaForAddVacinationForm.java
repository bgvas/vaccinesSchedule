...
public class InputAmkaForAddVacinationForm extends JDialog {
	...
	public static void main(String [] args) {
		try {
			...
			InputAmkaForAddVacinationForm dialog = new InputAmkaForAddVacinationForm();
	 ...
	 * Create the dialog.
	 */
	public InputAmkaForAddVacinationForm() {
		setBounds(610, 250, 249, 134);
		...
		saveBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				if(amkaVal.getText().length() < 11) {
					InputAmkaForAddVacinationForm.this.setVisible(false);
					amkaVal.setText("");
					amkaLbl.setVisible(false);
					errorMes.setVisible(true);
					InputAmkaForAddVacinationForm.this.setVisible(true);
				}
				
				String sql = "select * from pattients where amka = '" + amkaVal.getText() + "'";
				if(retrieve.checkIfAmkaExist(sql)) {
					InputAmkaForAddVacinationForm.this.setVisible(false);
					AddVaccinationDoneForm.main(amkaVal.getText());
				}
				else {
						InputAmkaForAddVacinationForm.this.setVisible(false);
						amkaVal.setText("");
